...
export class ShTranslateDirective implements AfterViewChecked, OnDestroy {
  ...
  @Input() set translateParams(params: any) {
    if (!_.isEqual(this.currentParams, params)) {
  ...
  updateValue(key: string, node: any, translations: any) {
    if (key) {
      ...
      let onTranslation = (res: string) => {
        ...
        node.currentValue = !isNoU(res) ? res : (node.originalContent || key);
        // we replace in the original content to preserve spaces that we might have trimmed
        this.setContent(node, this.key ? node.currentValue : node.originalContent.replace(key, node.currentValue));
        this._ref.markForCheck();
      };

      if (!isNoU(translations)) {
  ...
  getContent(node: any): string {
    return !isNoU(node.textContent) ? node.textContent : node.data;
  }

  setContent(node: any, content: string): void {
    if (!isNoU(node.textContent)) {
