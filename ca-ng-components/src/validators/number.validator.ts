...
function isInvalid(control: ShFormControl<string>, parser: NumberParserService, allowNegative: boolean, decimalPlaces: number, format: string) {
...
export function number(allowNegative: boolean, decimalPlaces: number, format: string, translateService: TranslateService) {
  ...
  return (control: ShFormControl<string>) => {
