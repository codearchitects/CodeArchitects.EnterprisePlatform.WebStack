import {
  ...
  AfterContentChecked, AfterContentInit, ChangeDetectorRef, SimpleChange
} from '@angular/core';
...
import { ShStorageService } from '../services/storage.service';
/**
...
export abstract class ActionShellComponent<
   ...
   */
  // codice remmatto per problema torna indietro
  // public saveState() {
  ...
  //       this.delegates.savePayload(this.activity.payload)
  //         .subscribe((success) => {
  //           if (success) {
  ...
  //   } else {
  //     /* tslint:disable */
  //     // debugger; // please notify if holds here
  //     /* tslint:enable */
  //   }
  // }
  public async saveState() {
    if (this.activity && this.activity.taskId) {
      if (this.activity.payload) {
        const currentTaskKey = 'taskslot_' + this.activity.taskId;
        const currentTaskData = this.delegates.serializer.serialize(this.activity.payload);
        ShStorageService.initialize(this.delegates.serializer);
        ShStorageService.instance.setItem(currentTaskKey, currentTaskData);
        /**
         * try to save on host also
         */
        const taskId = this.activity.taskId;
