import { routeCanActivateGuard, routeCanDeactivateGuard } from './../services/route.guard';
  ...
  return MetadataHelpers.defineMetadata(ActivityComponentMetadataKey, args, (target, targetKey) => {
    ...
    if (actual) {
      ...
      if (_path instanceof Array) {
        _path.forEach(p => __taskRouteItems[actual.task][p] = {
          path: p,
          component: target,
          canActivate: (args.canActivate || actual.canActivate) && [routeCanActivateGuard()],
          canDeactivate: (args.canDeactivate || actual.canDeactivate) && [routeCanDeactivateGuard()]
        });
    ...
    if (path instanceof Array) {
      path.forEach(p => __taskRouteItems[args.task][p] = {
        path: p,
        component: target,
        canActivate: args.canActivate && [routeCanActivateGuard()],
        canDeactivate: args.canDeactivate && [routeCanDeactivateGuard()]
      });
 ...
 */
export function getRoutesByTask(task: string, canActivate: Array<CanActivateFn | ProviderToken<any>> = [], canDeactivate: Array<CanDeactivateFn<any> | ProviderToken<any>> = []) {
