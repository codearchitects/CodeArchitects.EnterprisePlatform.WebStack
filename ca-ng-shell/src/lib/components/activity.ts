...
export abstract class Activity<TPayload extends IActivityPayload> {
  ...
  public abstract onSavePayload(payload: TPayload): Observable<boolean | null>;
  public abstract onLoadPayload(isWakingUp?: boolean): Observable<TPayload | null>;
          ...
          try {
            ...
            setTimeout(() => {
              this.onSavePayload(<any>this.payload).subscribe(result => {
                ...
                this.navigate({ name: newState, params: null, ...(this.useQueryParams ? { queryParams: params } : { params }) }, observer);
              });
            }, 100);
          } catch (e) {
            observer.next(false);
  ...
  */
   ...
   */
  protected navigate({ name, params, queryParams }: IShRoute, observer?: Subscriber<boolean>) {
    let deferred: Promise<boolean>;
    this.onLoadPayload()
      .subscribe(payload => {
        this.payload = payload;
        if (params && params.length) {
          deferred = this.router.navigate([this._baseRoutePath + name, ...params], { replaceUrl: false, queryParams });
        } else {
          deferred = this.router.navigate([this._baseRoutePath + name], { replaceUrl: false, queryParams });
        }
        deferred
          .then(fullFilled => {
            if (fullFilled) {
