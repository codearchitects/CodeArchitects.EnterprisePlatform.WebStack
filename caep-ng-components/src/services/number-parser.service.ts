...
@Injectable()
export class CaepNumberParserService implements OnDestroy {
  /**
  ...
  constructor(private translate: TranslateService) {
    ...
    this.translate.onLangChange.pipe(takeUntil(this.destroy$)).subscribe(r => {
  ...
  /*protected*/ public tolerantRegex(allowNegative: boolean, decimalPlaces: number) {
    const regexBuilder =
      '^' +
      '(' +
      this.negativeNumberRegex(allowNegative) +
      '\\d*)' +
